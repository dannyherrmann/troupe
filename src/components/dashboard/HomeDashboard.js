...
export const HomeDashboard = () => {
    ...
    function classNames(...classes) {
      ...
      const deleteButtonConfirm = (eventId, active) => {
        ...
        console.log(`here is delete event id`,deleteEventId)
        return (
          <a
          href="#"
          ...
          <a
                  ...
                  onClick={() => {
                    const fetchEvent = async () => {
                      ...
                      setEditSelectedEventType({id: event[0].eventType.id, name: event[0].eventType.name})
    ...
    const user = {
      ...
      const editEvent = (event) => {
        ...
        const saveEvent = async () => {
          ...
          const response = await fetch(`http://localhost:8088/events/${editEventId}`, options)
          await response.json()
          fetchEvents()
...
<div className="py-10">
        ...
        <table className="min-w-full divide-y divide-gray-300">
          ...
          <tbody>
            {events.map((event, eventIdx) => (
              <tr id={event.id} key={event.id} ref={deleteButtonRef}>
                <td
                  className={classNames(
  ...
  <Transition.Root show={open} as={Fragment}>
    ...
    <Transition.Root show={deleteAlert} as={Fragment}>
            <Dialog as="div" className="relative z-10" initialFocus={deleteButtonRef} onClose={setDeleteAlert}>
              <Transition.Child
                as={Fragment}
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            const deleteEvent = async () => {
                              ...
                              await fetch(`http://localhost:8088/events/${deleteEventId}`, options);
                              fetchEvents();
        ...
        <Dialog as="div" className="relative z-10" onClose={setOpenEditEvent}>
          <div className="fixed inset-0" />
                      ...
                      <div className="flex-1">
                        ...
                        <div className="space-y-6 py-6 sm:space-y-0 sm:divide-y sm:divide-gray-200 sm:py-0">
                          ...
                          <div className="space-y-1 px-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:space-y-0 sm:px-6 sm:py-5">
                            ...
                            <Listbox value={editSelectedEventType} onChange={setEditSelectedEventType}>
        {({ open }) => (
          <>
          
            <div className="relative mt-1">
              <Listbox.Button className="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
                <span className="block truncate">{editSelectedEventType.name}</span>
                <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                  <ChevronUpDownIcon className="h-5 w-5 text-gray-400" aria-hidden="true" />
                </span>
              </Listbox.Button>
